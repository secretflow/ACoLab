...
class Generateresponse:
    ...
    def process(self):
        ...
        for data in tqdm(self.data):
            ...
            prompt = f"你是一个可用于医疗对话文本生成的智能助手。请仔细阅读并分析以下医患对话文本，它只包含患者的问题。你的任务是根据以下四项医生回复生成的要求及其相应的定义来生成问题的回复。\n\n \
            **患者的问题**：\n \
            {text}\n\n \
            **医生回复生成的要求**：\n \
            1. **回答的准确性**：医生应能准确理解患者的问题，并提供科学、准确的回答。\n \
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="/mnt/data3/nianke_multi_agent/agentscope/data_process_api/data_generate/train_medical_sample/train_medical_sample_generate_question.jsonl")
    ...
    #############找种子数据############
    seed_data_a = random.sample(dataset,2000)
    seed_data_a = [element for element in seed_data_a if element['conversations'][0]['value'] != '无' and element['conversations'][1]['value'] != '无']

    generate_response_index = []

    generate_response =  []

    for i in range(len(dataset)):
        if dataset[i]['conversations'][1]['value'] == '无':
            generate_response_index.append(i)
            generate_response.append(dataset[i])

    if len(generate_response) > 0:

        gr = Generateresponse(generate_response,seed_data_a)
        result = gr.process()
        i = 0
        for idx in generate_response_index:
            if '**生成的回复**：' in result[i]:
                response = result[i].split('**生成的回复**：')[1].strip()
            else:
                response = '无'
    ...
    with open('/mnt/data3/nianke_multi_agent/agentscope/data_process_api/data_generate/train_medical_sample/train_medical_sample_generate.jsonl', 'w', encoding='utf-8') as file:
