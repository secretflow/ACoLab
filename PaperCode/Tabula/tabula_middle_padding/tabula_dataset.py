    ...
    def _getitem(
        ...
        for i in idx_range:
            ...
            #     # data_row_text_list.append("%s" % (str(row.columns[i].to_pylist()[0]).strip()))
                ...
                data_row_text_list.append(
                    "%s" % (str(row.columns[i].to_pylist()[0]).strip())
                )
        # shuffled_text = ", ".join(
...
class TabulaDataCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        for i in range(len(self.token_list_length)):
            sublist = [
                item[i] for item in features
            ]  # sublist is a list of all the encoding for one data column
        ...
        for i in range(number_sentences):
            ...
            temp = [
                item for sublist in sentence for item in sublist
            ]  # flatten the list
            encoded_text.append(temp)

        batch = {
            "input_ids": torch.Tensor(encoded_text).long(),
            "attention_mask": torch.ones(torch.Tensor(encoded_text).shape).long(),
        }
