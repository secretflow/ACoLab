...
from model.ella import load_ella,ELLAProxyUNet
...
def parse_args():
    ...
    ## ELLA
    parser.add_argument('--ella_path', help='path of ella', type=str, required=False)
...
def main():
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        if args.ella_path:
            ...
            ref_text_encoder = T5EncoderModel.from_pretrained(args.t5_path)
            ella = load_ella(args.ella_path, unet.device, unet.dtype)
            unet = ELLAProxyUNet(ella, unet)
        else:
            text_encoder = CLIPTextModel.from_pretrained(
                args.pretrained_model_name_or_path, subfolder="text_encoder", revision=args.revision
            )
            ref_text_encoder = CLIPTextModel.from_pretrained(
                args.pretrained_model_name_or_path, subfolder="text_encoder", revision=args.revision
            )
    ...
    if args.train_method == 'dpo': 
        ...
        ref_text_encoder.requires_grad_(False)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            ...
            for i in range(len(models_to_load)):
                ...
                load_model = UNet2DConditionModel.from_pretrained(input_dir, subfolder="text_encoder")
    ...
    def tokenize_captions(examples, is_train=True):
        ...
        if args.ella_path:
            # Tokenize the captions with the T5 tokenizer
            inputs = tokenizer(
    ...
    for param in ref_text_encoder.parameters():
    ...
    #pytorch_total_params = sum(p.numel() for p in text_encoder.parameters())
    #print('Parameters:', pytorch_total_params)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.ella_path:
                ...
                ref_encoder_hidden_states = ref_text_encoder(batch["input_ids"]).last_hidden_state
            else:
                encoder_hidden_states = text_encoder(batch["input_ids"])[0]
                ref_encoder_hidden_states = ref_text_encoder(batch["input_ids"])[0]
            if args.train_method == 'dpo':
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        unwrapped_text_encoder = accelerator.unwrap_model(unet)
